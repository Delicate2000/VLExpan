...
class BEiT3ForVisualReasoning(BEiT3Wrapper):
    ...
    def forward(self, image_a, text_description, padding_mask, **kwargs):
        ...
        vision_input = image_a
        language_input = text_description
...
class BEiT3ForCaptioning(BEiT3Wrapper):
    ...
    def forward(self, image, text_ids, padding_mask, language_masked_pos=None, text_len=None, incremental_state=None, **kwargs):
...
class BEiT3ForPretrain(BEiT3Wrapper):
    def __init__(
            self, 
            args, 
            **kwargs
    ):
        super(BEiT3ForPretrain, self).__init__(args=args)
        # embed_dim = args.encoder_embed_dim
        # self.mlm_head = nn.Linear(embed_dim, args.vocab_size)
        # self.mlm_head.apply(self._init_weights)
...
@register_model
def beit3_large_pretain(pretrained=False, **kwargs):
    args = _get_large_config(img_size=224, **kwargs)
    model = BEiT3ForPretrain(args, **kwargs)
    return model
